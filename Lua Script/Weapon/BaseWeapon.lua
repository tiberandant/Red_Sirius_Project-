...
function BaseWeapon:onFired(firingPlayer, fireInfo, fromNetwork)
	if not IsServer then
		...
		-- Do not simulate weapon fire if the streaming is enabled and the firing player is not within current player's streaming area.
		if not firingPlayer.character:FindFirstChild("HumanoidRootPart") then
			return
		end
	...
	self.connections.reload = RunService.Heartbeat:Connect(function()
		...
		if self.ammoInWeaponValue then
			local success, errMsg = pcall(function()
				local usedAmmo = weapon.Configuration.AmmoCapacity.Value - weapon.CurrentAmmo.Value
				if weapon.Configuration.ReservedAmmo.Value > usedAmmo then
				self.ammoInWeaponValue.Value = self:getConfigValue("AmmoCapacity", 30)
				weapon.Configuration.ReservedAmmo.Value -= usedAmmo
				elseif weapon.Configuration.ReservedAmmo.Value <= usedAmmo then
					local ammoLeft = weapon.Configuration.ReservedAmmo.Value + weapon.CurrentAmmo.Value
					weapon.CurrentAmmo.Value = ammoLeft
					weapon.Configuration.ReservedAmmo.Value = 0
				end
			end)			
		end

		if self.connections.reload then
			if weapon.Configuration.ReservedAmmo.Value == 0 then
			   weapon.Configuration.AmmoCapacity.Value = 0
			end
			self.connections.reload:Disconnect()
			self.connections.reload = nil
		end
